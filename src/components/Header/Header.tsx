    ...
    return (
        <header className="main-header">
            <Navbar expand="lg" className="navbar-custom w-100">
                <Container fluid className="px-lg-4 py-2">
                    {/* Logo Section */}
                    ...
                    {/* Mobile Toggle */}
                    <Navbar.Toggle aria-controls="lumea-navbar-nav" className="border-0 shadow-none">
                        <span className="navbar-toggler-icon" style={{ filter: 'invert(1)' }}></span>
                    </Navbar.Toggle>

                    {/* Navbar Items */}
                    <Navbar.Collapse id="lumea-navbar-nav">
                        {/* Center Links - Unified */}
                        <Nav className="mx-auto nav-links-container align-items-lg-center">
                            <Link className="nav-link-item" to="/">Skin Care</Link>
                            <Link className="nav-link-item" to="/maquiagem">Maquiagem</Link>
                            <Link className="nav-link-item" to="/produtos/corpo&banho">Corpo & Banho</Link>
                            <Link className="nav-link-item" to="/produtos/fragrancias">Fragrâncias</Link>
                            <Link className="nav-link-item" to="/produtos/cabelo">Cabelo</Link>
                            <Link className="nav-link-item" to="/produtos/homem">Homem</Link>
                        </Nav>

                        {/* Right Section - Search and Action Buttons */}
                        <Nav className="align-items-lg-center gap-3 right-actions-container">
                            <div className="busca-container">
                                <svg className="icone_lupa" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" onClick={handleSearch}>
                            ...
                            {isLoggedIn ? (
                                <div className="botoes_direita">
                                    <Link to="/produtos/cadastro" title="Cadastrar Produtos" className="btn-action">
                                    ...
                                    <button onClick={handleLogout} title="Sair / Finalizar sessão" className="btn-action btn-logout-icon">
                                ...
                                <div className="botoes_direita">
                                    <Link to="/login" title="Fazer login" className="btn-action">
